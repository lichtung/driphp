...
class AES
{
    ...
    /**
     * @param string $key 加密解密的钥匙
     * @param string $method 加密方法,默认为aes256
     * @return AES
     */
    public static function getInstance($key, $method = self::METHOD_AES_256_CFB)
    {
        static $_instances = [];
        $key = md5(serialize([$key, $method]));
        if (!isset($_instances[$key])) {
            $_instances[$key] = new static($key, $method);
        }
     ...
     * @param string $method
     */
    private function __construct($key, $method = self::METHOD_AES_256_CFB)
    {
        $this->_key = $key;
