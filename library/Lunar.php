...
class Lunar
{
     ...
     * @return int
     */
    public static function getLastDayOfThisMonth(int $year = null, int $month = null): int
    {
        if (null === $year) $year = 'Y';
        if (null === $month) $month = 'm';
        return (int)date('d', strtotime(date("{$year}-{$month}-01") . ' +1 month -1 day'));
    }
     ...
     * @return array|string
     */
    public static function convertSolarToLunar()
    {
        //debugger;
        $year = date('Y');
        $month = date('n');
        $date = date('j');
        $yearData = self::LUNAR[$year - self::MIN_YEAR];
        if ($year == self::MIN_YEAR && $month <= 2 && $date <= 9) {
            return array(1891, '正月', '初一', '辛卯', 1, 1, '兔');
        }
        $aa = self::getLunarByBetween($year, self::getDaysBetweenSolar($year, $month, $date, $yearData[1], $yearData[2]));
        return date('Y-m-d H:i:s', time()) . "\n" . '农历:' . $aa[3] . '[' . $aa[6] . ']年' . $aa['1'] . $aa[2];
    }
