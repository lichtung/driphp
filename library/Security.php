 ...
 *
 * @method Security getInstance(string $index = '') static
 *
 */
class Security extends Component
{
     ...
     * @var    array
     */
    private static $filename_bad_chars = array(
        '../', '<!--', '-->', '<', '>',
        "'", '"', '&', '$', '#',
        '{', '}', '[', ']', '=',
        ';', '?', '%20', '%22',
    ...
    protected function initialize()
    {
        ...
        if ($this->config['csrf_protection']) {
            ...
            if ($cookie_prefix = $this->config['cookie_prefix']) {
        ...
        $this->charset = strtoupper($this->config['charset']);
...
//                if (preg_match('#^' . $excluded . '$#i' . (/*UTF8_ENABLED*/
//                    true ? 'u' : ''), $uri->uri_string())) {
//                    return $this;
//                }
//            }
//        }
    ...
    public function csrf_set_cookie()
    {
        ...
        if ($this->config['cookie_secure'] && !Request::isHttps()) {
            return false;
        }
    ...
    {
//        Response::sendCode(403, 'The action you have requested is not allowed.');
        exit;
    }
...
//        }
        ...
        if (is_readable('/dev/urandom') && ($fp = fopen('/dev/urandom', 'rb')) !== FALSE) {
            ...
            stream_set_chunk_size($fp, $length);// current version of PHP is equal to or greater than 5.4
    ...
    public function entity_decode($str, $charset = NULL)
    {
        ...
        if (!isset($_entities)) {
            $_entities = array_map('strtolower', call_user_func_array('get_html_translation_table', [
                HTML_ENTITIES, $flag, $charset
            ]));
